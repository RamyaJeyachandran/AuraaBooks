...
orders_path = 'd:/AuraaZenAIProject/abproject/templates/sales_orders.html'
dc_path = 'd:/AuraaZenAIProject/abproject/templates/sales_delivery_challan.html'

with open(orders_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Replacements to convert Sales Order -> Delivery Challan
content = content.replace('Sales Order', 'Delivery Challan')
content = content.replace('Sales Orders', 'Delivery Challans')
content = content.replace('sales_orders', 'sales_delivery_challan')
content = content.replace('Order Date', 'Challan Date')
content = content.replace('ORD-', 'DC-')
content = content.replace('rowActionMenu_ORD', 'rowActionMenu_DC')
content = content.replace('saveOrder()', 'saveDeliveryChallan()')

# We need to add the POS View Modal
# Find where quickViewModal ends and insert posViewModal right after it.
# Or better, just inject posViewModal at the end of the file before <script>

pos_full_html = """
<div id="posViewModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[9999] hidden items-center justify-center p-4 md:p-8">
...
# Insert pos_full_html right before {% block extra_scripts %}
block_script_idx = content.find('{% block extra_scripts %}')
if block_script_idx != -1:
    content = content[:block_script_idx] + pos_full_html + '\n' + content[block_script_idx:]

# Also, update selectViewMode JS function
...
start_js = content.find('function selectViewMode')
end_js = content.find('function filterByStatus', start_js)
if start_js != -1 and end_js != -1:
    content = content[:start_js] + new_selectViewMode + content[end_js:]

# We need to make sure the "POS View" button is added to the Dropdown if it's missing.
# Normal view dropdown:
pos_btn = """                                    <button type="button" onclick="selectViewMode('POS View')" class="w-full text-left px-4 py-2.5 text-[13px] font-bold text-slate-700 hover:bg-[var(--accent)] hover:text-white transition-all flex items-center gap-2 group border-none bg-transparent cursor-pointer">
                                        <svg class="w-4 h-4 text-slate-500 group-hover:text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="3" width="20" height="18" rx="2" ry="2"/><line x1="2" y1="8" x2="22" y2="8"/><line x1="8" y1="18" x2="8" y2="8"/></svg>
                                        POS View
                                    </button>"""
if 'selectViewMode(\'POS View\')' not in content:
    # Add to normal dropdown
    qv_btn_normal = """                                    <button type="button" onclick="selectViewMode('Quick View')" class="w-full text-left px-4 py-2.5 text-[13px] font-bold text-slate-700 hover:bg-[var(--accent)] hover:text-white transition-all flex items-center gap-2 group border-none bg-transparent cursor-pointer">
                                        <svg class="w-4 h-4 text-slate-500 group-hover:text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
                                        Quick View
                                    </button>"""
    content = content.replace(qv_btn_normal, qv_btn_normal + '\n' + pos_btn)

    # Add to Quick view dropdown
    qv_btn_qv = """                                <button type="button" onclick="selectViewMode('Quick View')" class="w-full text-left px-4 py-2.5 text-[13px] font-bold text-slate-700 hover:bg-[var(--accent)] hover:text-white transition-all flex items-center gap-2 group border-none bg-transparent cursor-pointer">
                                    <svg class="w-4 h-4 text-slate-500 group-hover:text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
                                    Quick View
                                </button>"""
    pos_btn_qv = pos_btn.replace('                                    <button', '                                <button')
    content = content.replace(qv_btn_qv, qv_btn_qv + '\n' + pos_btn_qv)

# Replace new_sales_order with new_sales_delivery_challan route if applicable
content = content.replace('{% url \'new_sales_order\' %}', '{% url \'new_sales_delivery_challan\' %}')

with open(dc_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Restored sales_delivery_challan.html successfully.")
